...
public class DilophosaurusEntity extends Animal implements IAnimatable {
	...
	public DilophosaurusEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(FossilsoriginsModEntities.DILOPHOSAURUS.get(), world);
	}
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "dilophosuarus");
	}
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		DilophosaurusEntity retval = FossilsoriginsModEntities.DILOPHOSAURUS.get().create(serverWorld);
	...
	private <E extends IAnimatable> PlayState movementPredicate(AnimationEvent<E> event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.Dilophosaurus.move", EDefaultLoopTypes.LOOP));
				return PlayState.CONTINUE;
			}
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.Dilophosaurus.idle", EDefaultLoopTypes.LOOP));
			return PlayState.CONTINUE;
		}
	...
	private <E extends IAnimatable> PlayState attackingPredicate(AnimationEvent<E> event) {
		...
		if (this.swinging && event.getController().getAnimationState().equals(software.bernie.geckolib3.core.AnimationState.Stopped)) {
			...
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.Dilophosaurus.attack", EDefaultLoopTypes.PLAY_ONCE));
			return PlayState.CONTINUE;
		}
		return PlayState.CONTINUE;
	}
