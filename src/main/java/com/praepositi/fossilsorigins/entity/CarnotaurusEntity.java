...
public class CarnotaurusEntity extends Animal implements IAnimatable {
	...
	public CarnotaurusEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(FossilsoriginsModEntities.CARNOTAURUS.get(), world);
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(2, new AvoidEntityGoal<>(this, SpinosaurusEntity.class, (float) 6, 1, 1.2));
		this.targetSelector.addGoal(3, new NearestAttackableTargetGoal(this, Player.class, false, true) {
			@Override
			public boolean canUse() {
				double x = CarnotaurusEntity.this.getX();
				double y = CarnotaurusEntity.this.getY();
				double z = CarnotaurusEntity.this.getZ();
				Entity entity = CarnotaurusEntity.this;
				Level world = CarnotaurusEntity.this.level;
		...
		this.targetSelector.addGoal(12, new NearestAttackableTargetGoal(this, SpinosaurusEntity.class, false, true) {
			@Override
			public boolean canUse() {
				double x = CarnotaurusEntity.this.getX();
				double y = CarnotaurusEntity.this.getY();
		...
		this.goalSelector.addGoal(14, new RandomStrollGoal(this, 2));
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		CarnotaurusEntity retval = FossilsoriginsModEntities.CARNOTAURUS.get().create(serverWorld);
	...
	private <E extends IAnimatable> PlayState movementPredicate(AnimationEvent<E> event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.Carnotaurus.move", EDefaultLoopTypes.LOOP));
				return PlayState.CONTINUE;
			}
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.Carnotaurus.idle", EDefaultLoopTypes.LOOP));
			return PlayState.CONTINUE;
		}
	...
	private <E extends IAnimatable> PlayState attackingPredicate(AnimationEvent<E> event) {
		...
		if (this.swinging && event.getController().getAnimationState().equals(software.bernie.geckolib3.core.AnimationState.Stopped)) {
			...
			event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.Carnotaurus.attack", EDefaultLoopTypes.PLAY_ONCE));
			return PlayState.CONTINUE;
		}
		return PlayState.CONTINUE;
	}
